...
export const RPS_GAME_Content = {
  ...
  shoot: (payload: any) => {
    ...
    ///User Match winner
    if (match_over && match_winner === 'user')
      return build_user_winner_phrase(username);

    ///kitty Match winner
    if (match_over && match_winner === 'kitty')
      return build_kitty_winner_phrase();

    ///User Round Winner
    if (winner === 'user')
      return `${username} WON this round! \n**Rounds ${rounds}/5**  ---  ${username} - ${currentUserWinCount} | kitty chan - ${currentKittyWinCount}`;
    ///kitty Game Winner
    if (winner === 'kitty')
      return `kitty chan WON this round! \n**Rounds ${rounds}/5**  ---  ${username} - ${currentUserWinCount} | kitty chan - ${currentKittyWinCount}`;
  },
};
