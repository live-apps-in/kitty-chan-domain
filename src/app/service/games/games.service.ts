...
import { RPS_WAKE_ACTIONS } from '../../data/games/rpsGame';
...
export class GamesService{
	...
	async validateGame(guild: IGuild): Promise<any> {
		...
		///Check Session
		const gameSession = await GameSession.findOne({ threadId: guild.channelId });
	...
	private async gameFactory(messageChunk: string[], guild: IGuild) {
		...
		if (messageChunk[2] === 'rock' && messageChunk[3] === 'paper' && messageChunk[4] === 'scissors') {
