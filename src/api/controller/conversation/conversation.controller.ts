...
export class ConversationController {
  constructor(
    @inject(TYPES.ConversationAPIService)
    private readonly conversationService: ConversationAPIService,
  ) {}
  ...
  async addConversationPhrase(req: Request) {
    const savePhrase = await this.conversationService.add_conversation_phrase(
      req.body,
    );
    return savePhrase;
  }
  ...
  async getConversationPhraseByTag(@requestParam('tagName') tag: string) {
    const getPhrase =
      await this.conversationService.view_conversation_phrase_by_tag(tag);
    return getPhrase;
  }
  ...
  async addConversationResponse(req: Request) {
    const savePhrase = await this.conversationService.add_conversation_response(
      req.body,
    );
    return savePhrase;
  }
  ...
  async getConversationResponse() {
    const getPhrase =
      await this.conversationService.view_conversation_response();
    return getPhrase;
  }
  ...
  async getConversationResponseByTag(@requestParam('tagName') tag: string) {
    const getPhrase =
      await this.conversationService.view_conversation_response_by_tag(tag);
    return getPhrase;
  }
